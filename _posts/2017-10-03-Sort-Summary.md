...
**快速排序的改进——快速排序的随机化版本**
在这种方法中，不是始终采用A[r](r为最末尾的那个元素)作为主元（pivot），而是从字数组A[p,,r]中随机选择一个元素，即将A[r]与从A[r]中随机选出的那个元素交换。这么做确保了在子数组中r-p+1个元素中，主元素A[r]等可能取其中的任何一个。因为主元素是随机选择的，我么期望在平均情况下，对输入数组的划分能比较对称。
算法实现：
